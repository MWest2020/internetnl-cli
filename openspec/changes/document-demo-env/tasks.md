...
- [ ] 2.1 Add a `NETNL_DEMO_*` block to `deploy/.env.example`, following the
      shape of the blocks already in that file: a short header comment, then
      commented-out `NAME=default` lines.
- [ ] 2.2 Cover all ten names read by `netnl/settings.py`:
...
- [ ] 2.3 Every default written in the file MUST match the default in
      `src/netnl/settings.py`. Read them from the source; do not guess.
- [ ] 2.4 Say in the header comment that the family is opt-in and that
      `NETNL_DEMO_ENABLED`, `NETNL_DEMO_ALLOWED_ORIGIN` and
      `NETNL_DEMO_TENANT` are the three needed for the route to exist.
- [ ] 2.5 Leave `NETNL_PUBLIC_HOST` and `NETNL_UPSTREAM_NETWORK` untouched —
...
- [ ] 3.1 `sh scripts/verify.sh` in the run output (nothing should change,
      but the gate must be seen to pass)
- [ ] 3.2 The rendered diff of `deploy/.env.example` in the run output
