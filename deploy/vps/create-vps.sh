...
# the Tailscale auth key and the operator's SSH public key substituted
# in, uses that as the server's user-data, and deletes the temp file
# again — the rendered cloud-init (and therefore the auth key) is never
# written anywhere else, never printed and never committed.
#
# Required environment (never echoed by this script):
#   HCLOUD_TOKEN    Hetzner Cloud API token (project-scoped).
#   TS_AUTHKEY      Tailscale pre-auth key. Use an EPHEMERAL, TAGGED key
#                   (Tailscale admin console -> Settings -> Keys) so it
#                   cannot be reused to register another device and is
#                   scoped by ACL tag, not by a long-lived user identity.
...
sed \
    ...
    -e "s|\${OPERATOR_SSH_PUBLIC_KEY}|$ssh_public_key|g" \
